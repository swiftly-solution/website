...
### Clear Events
If your API contains events that can be registered by other plugins, 
make sure to set them to null or create a new API object to provide to the interfaceManager each time ConfigureSharedInterface is called.
Otherwise, events may be registered multiple times.
...
See [IInterfaceManager](/docs/api/iinterfacemanager/) for more details.