...
public void OnInfoCommand(ICommandContext context)
{
    ...
    {
        var player = context.Sender!;
        context.Reply($"You are {player.Name}");
    }
    else
    {
...
{
    var player = Core.PlayerManager.GetPlayerByIndex(playerId);
...
public HookResult OnChatFilter(int playerId, string text, bool teamonly)
{
    ...
    // Log all chat
    Console.WriteLine($"[{(teamonly ? "TEAM" : "ALL")}] {player.Name}: {text}");
...
See [ICommandService](/docs/api/shared/commands/icommandservice) and [ICommandContext](/docs/api/shared/commands/icommandcontext) for more details.